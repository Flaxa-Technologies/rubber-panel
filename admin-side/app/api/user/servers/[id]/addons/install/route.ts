...
export async function POST(
  ...
  try {
    ...
    const access = await verifyServerAccess(id, userId, "file.create");
    if (!access.allowed) {
    ...
    const nodeRes = await fetch(`${baseUrl}/api/agent/servers/${id}/addons/install`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ error: err.message || "Failed to proxy addon installation" }, { status: 500 });
  }
}
