...
export async function GET(
  ...
  try {
    ...
    const nodeRes = await fetch(`${baseUrl}/api/agent/servers/${id}/properties`, {
      headers: {
...
export async function PUT(
  ...
  try {
    ...
    const access = await verifyServerAccess(id, userId, "file.edit");
    if (!access.allowed) {
      return NextResponse.json({ error: access.error || "Access denied" }, { status: 403 });
    }
    ...
    return NextResponse.json({ error: err.message || "Failed to save properties" }, { status: 500 });
  }
}
