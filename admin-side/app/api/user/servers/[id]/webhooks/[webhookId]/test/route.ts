...
export async function POST(
  ...
  try {
    ...
    const [server, webhook] = await Promise.all([
      db.server.findUnique({ where: { id }, include: { node: true } }),
      db.serverWebhook.findUnique({ where: { id: webhookId, serverId: id } }),
    ]);

    if (!server || !server.node || !webhook) {
      return NextResponse.json({ error: "Server, node, or webhook not found" }, { status: 404 });
    }

    const baseUrl = getNodeBaseUrl(server.node);
    const nodeRes = await fetch(`${baseUrl}/api/agent/servers/${id}/webhook/test`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json({ error: err.message || "Failed to trigger test webhook" }, { status: 500 });
  }
}
