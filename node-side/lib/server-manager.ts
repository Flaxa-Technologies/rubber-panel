...
export async function reinstallServer(serverId: string, options: ReinstallOptions = {}): Promise<{ success: boolean; error?: string }> {
  console.log(`[ServerManager] REINSTALL server ${serverId}`, options);
  appendLog(serverId, "[Rubber] Initiating server reinstallation...");

  // 1. Stop and remove container
  await stopServer(serverId, true).catch(() => {});
  await deleteContainer(serverId).catch(() => {});

  const dir = getServerDir(serverId);
  await fs.mkdir(dir, { recursive: true });

  // 2. Selectively wipe directory
  const preserveList = (options.preservePaths || [])
    .map(p => p.replace(/^[/\\]+/, "").trim().toLowerCase())
    .filter(Boolean);

  const shouldPreserve = (entryName: string): boolean => {
    const low = entryName.toLowerCase();
    if (isSystemProtectedPath(entryName)) return true;
    return preserveList.some(p => {
      return low === p || low.startsWith(p + "/") || p.startsWith(low + "/");
    });
  };
  ...
  try {
    ...
    console.error(`[ServerManager] Error cleaning server dir for ${serverId}:`, err);
  }
  ...
  if (!info) {
    info = {
      id: serverId,
      name: `server-${serverId.slice(0, 8)}`,
      status: "STOPPED",
      ram: 2048,
      cpu: 100,
      port: 25565,
  ...
  const assignedPort = info.port || 25565;

  // 4. Engine-specific reinstallation
  if (runtime.isPumpkin) {
    const javaPort = assignedPort;
    const bedrockPort = env.BEDROCK_PORT ? parseInt(env.BEDROCK_PORT, 10) : (assignedPort + 1000);
    try {
      const { ensurePumpkinConfiguration, installPumpkinBinaryOnNode } = await import("./pumpkin-agent");
      await ensurePumpkinConfiguration(dir, javaPort, bedrockPort);
      const versionId = env.VERSION || options.softwareVersion || "pumpkin-nightly-latest";
      await installPumpkinBinaryOnNode({ versionId, commitSha: "nightly" });
      appendLog(serverId, `[Rubber] ✓ Pumpkin MC engine and configuration reinstalled successfully.`);
    } catch (pErr: any) {
      appendLog(serverId, `[Rubber] Warning preparing Pumpkin binary: ${pErr?.message}`);
    }
  } else if (runtime.isMinecraft) {
    // EULA
    ...
    if (!fsSync.existsSync(propPath)) {
      const defaultProp = [
        "# Minecraft server properties",
        "# Auto-generated by Rubber Panel Reinstall",
        `server-port=25565`,
        `query.port=25565`,
        "enable-rcon=false",
        "online-mode=false",
      ...
      await fs.writeFile(propPath, defaultProp, "utf-8").catch(() => {});
    }

    // Target software & version
    const swType = options.softwareType || env.TYPE || "PAPER";
    const swVer = options.softwareVersion || env.VERSION || "1.21.1";
    appendLog(serverId, `[Rubber] Downloading fresh Minecraft server software: ${swType} ${swVer}...`);
    try {
      await downloadMinecraftJarIfMissing(serverId, swType, swVer);
    } catch (dErr: any) {
      appendLog(serverId, `[Rubber] Note downloading jar: ${dErr?.message}`);
    }
    appendLog(serverId, `[Rubber] ✓ Minecraft (${swType} ${swVer}) reinstallation completed.`);
  } else if (runtime.isNodeJs) {
    const pkgPath = path.join(dir, "package.json");
    if (!fsSync.existsSync(pkgPath)) {
      await fs.writeFile(pkgPath, JSON.stringify({
        name: info.name.toLowerCase().replace(/[^a-z0-9-_]/g, "-") || "nodejs-app",
        version: "1.0.0",
        main: "server.js",
    ...
    const serverJsPath = path.join(dir, "server.js");
    if (!fsSync.existsSync(serverJsPath)) {
      await fs.writeFile(serverJsPath, `// Rubber Panel Node.js\nconst http = require('http');\nconst port = process.env.PORT || ${assignedPort};\nhttp.createServer((q,r)=>{r.end('Rubber Panel Node.js');}).listen(port);\n`).catch(() => {});
    }
  }
  ...
  const keptText = preserveList.length > 0 ? ` (Preserved: ${preserveList.join(", ")})` : " (Clean wipe)";
  appendLog(serverId, `[Rubber] ✓ Server reinstallation finished${keptText}. Press Start to initialize.`);
