...
const TAG = "v0.1.0-beta.30";
const NAME = "Rubber Panel v0.1.0-beta.30 — Pumpkin config/ Directory Discovery";
const BODY = `## What's New in v0.1.0-beta.30

### 🎃 Native Pumpkin config/ Directory Discovery
- **Official Path Resolution**: Pumpkin natively looks for its configuration inside the \`config/\` subdirectory (\`config/configuration.toml\`, \`config/config.toml\`).
- **All-Path Auto-Sync**: The node agent now pre-creates the \`config/\` directory and synchronizes:
  - \`config/configuration.toml\` (Primary Pumpkin engine path)
  - \`config/config.toml\`
  - \`config/Pumpkin.toml\`
  - \`config/features.toml\`
  - Root fallbacks (\`configuration.toml\`, \`config.toml\`)
- **Direct Port Binding**: Dynamically sets \`[networking.java]\` \`address = "0.0.0.0:\${javaPort}"\` and \`[networking.bedrock.nethernet]\` \`address = "0.0.0.0:\${bedrockPort}"\` so Pumpkin binds directly to allocated ports on boot!
`;
