...
const TAG = "v0.1.0-beta.33";
const NAME = "Rubber Panel v0.1.0-beta.33 — Host Hardware Telemetry & Unmetered Auto-Capacity";
const BODY = `## What's New in v0.1.0-beta.33

### 📊 Global Host Hardware Telemetry & Auto Capacity
- **Unmetered / Blank Node Capacity Logic**: When adding or managing nodes, leaving RAM or Storage capacity blank now automatically detects physical hardware RAM from the node agent and subtracts **1 GB (1024 MB)** reserved for host OS/system overhead (e.g. 8 GB RAM node -> 7.0 GB allocatable to servers). No more \`0.0 GB Allocated\`!
- **Dual RAM Monitoring**: Displays both **Rubber Server Allocations** (e.g. \`2.0 GB / 7.0 GB Allocated (29%)\`) and **Global Host Machine RAM** (e.g. \`5.2 GB / 7.8 GB used across entire OS (66% load)\`) so admins immediately know if external software is consuming host memory.
- **Dual Disk Telemetry**: Displays both actual **Game Server Directory Storage** (\`/var/rubber-panel/servers\`) and **Global Root Filesystem Disk Usage** (\`/\`).
- **Host CPU Activity**: Real-time load gauge tracking total host processor utilization.

### 🛠️ Engine & Dependency Fixes
- **Fixed \`geoip-lite\` EBADENGINE warning**: Switched to \`geoip-lite@^1.4.10\` for seamless native compatibility with Node.js 20 LTS.
- **Fast IP Detection**: Added \`--max-time 3\` timeout for public IP lookups in installation scripts to prevent installation stalling on airgapped or restricted networks.
