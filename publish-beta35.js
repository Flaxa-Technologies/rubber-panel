...
const TAG = "v0.1.0-beta.35";
const NAME = "Rubber Panel v0.1.0-beta.35 — Server Root Direct Render, Graceful API Resilience & Clean Next.js Cache";
const BODY = `## What's New in v0.1.0-beta.35

### ⚡ Direct Server Root Render & 0-Delay Navigation
- **Fixed \`/servers/[id]\` 500 SSR Error**: Replaced Server Component redirect with direct client render of \`ConsolePage\` at \`/servers/[id]\`, eliminating Next.js SSR redirect conflicts and ensuring instantaneous loading.
- **Graceful Player & Console API Handlers**: Fixed 500 / 503 errors when polling non-Minecraft / Code Sandbox / starting servers; now returns clean empty states (\`{ players: [], total: 0 }\` & \`{ lines: [], total: 0 }\`).
- **Clean Turbopack & Next Cache in Installer**: \`install-panel.sh\` and \`install-node.sh\` automatically flush \`.next\` cache before building to prevent stale route mismatches.
