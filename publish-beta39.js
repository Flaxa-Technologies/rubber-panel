...
const TAG = "v0.1.0-beta.39";
const NAME = "Rubber Panel v0.1.0-beta.39 — Container Hardening, Hidden System States & Minecraft RGB Isolation";
const BODY = `## What's New in v0.1.0-beta.39

### 🔒 Docker Container Hardening & Root Escalation Prevention
- **Non-Root & Dropped Capabilities**: Hardened Cloud Code Sandbox and application containers by enforcing \`--user 1000:1000\`, \`--security-opt=no-new-privileges:true\`, \`--pids-limit=256\`, and dropping critical kernel capabilities (\`SYS_BOOT\`, \`SYS_RAWIO\`, \`SYS_ADMIN\`, \`SYS_MODULE\`, \`SYS_PTRACE\`), preventing root escalation and harmful system reboots.

### 🛡️ Clean Isolated Server State Storage
- **Hidden & Protected System Files**: Migrated internal server metadata and \`.rp-state.json\` into isolated daemon storage (\`.data/server-states/\`) outside the user's workspace directory.
- **Strict File Manager & Code Sandbox Filtering**: Completely hidden all system files (\`.rp-state.json\`, \`.rp-lock\`, \`.cryo*\`, \`.git\`, etc.) and blocked read/write/delete attempts with 403 Forbidden.

### 🎨 Minecraft-Only RGB Gradient Tooling
- **Contextual Editor Tools**: Configured the RGB Gradient generator to display strictly for Minecraft instances, preventing clutter on non-Minecraft instances while retaining full formatting capabilities on Minecraft text and config files.
