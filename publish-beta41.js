...
const TAG = "v0.1.0-beta.41";
const NAME = "Rubber Panel v0.1.0-beta.41 — Live Docker Sync & Resilient State Recovery";
const BODY = `## What's New in v0.1.0-beta.41

### ⚡ Live Docker Health & Status Reconciliation
- **Accurate Real-Time Container State**: Automatically checks Docker inspect (\`containerRunning\`) during all status inquiries and heartbeat cycles. Instances running in Docker now immediately report \`RUNNING\` with live CPU/RAM utilization on both User and Admin panels.
- **Immediate Power Action Synchronization**: Starting, stopping, or restarting servers directly syncs the internal database state and daemon state in memory without waiting for delayed background intervals.

### 🛡️ Resilient Isolated State Recovery
- **Centralized State Resolver**: Implemented \`getOrLoadServerState\` which reliably reads isolated daemon state files (\`.data/server-states/\`), automatically migrates legacy state files if found, and recovers state dynamically if a container or folder is active.
- **Fixed False Offline Reports**: Eliminated missing state file exceptions on daemon reboots, guaranteeing that servers like Cloud Code Sandboxes are recognized and controllable.
