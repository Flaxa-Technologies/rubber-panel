...
const TAG = "v0.1.0-beta.49";
const NAME = "Rubber Panel v0.1.0-beta.49 — Self-Healing Power Engine, Code-Server Port Mapping & Strict Env Safeguards";
const BODY = `## Rubber Panel v0.1.0-beta.49

### 🚀 Highlights & Fixes
- **Self-Healing Server Launch Engine**: Automatically detects if a server container or state is missing on the node and auto-provisions it on-the-fly when starting from Admin or User panel.
- **Dynamic Node Authentication**: Enhanced \`node-side/lib/auth.ts\` to dynamically verify tokens and prevent 401/503 desyncs.
- **Standardized Cloud Code Sandbox (VS Code Web)**: Code-server containers now bind to internal \`:8080\` mapped to host ports to prevent non-root permission denied errors.
- **Strict Environment Safeguards**: Updaters and packaging scripts now strictly exclude and preserve production \`.env\` files and SQLite databases.
...
async function main() {
  ...
  try {
    ...
    execSync(`git commit -m "Release ${TAG} - Self-Healing Power Engine, Code-Server Port Mapping & Strict Env Safeguards"`, { stdio: "inherit" });
  } catch (e) {
