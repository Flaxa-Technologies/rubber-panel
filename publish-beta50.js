...
const TAG = "v0.1.0-beta.50";
const NAME = "Rubber Panel v0.1.0-beta.50 — [ Rubber ] Console Identity, RCON Auto-Execution & Single Prompt Stream";
const BODY = `## Rubber Panel v0.1.0-beta.50

### 🚀 Highlights & Fixes
- **Branded Console Identity**: Standardized all system, startup, and lifecycle log prefixes to \`[ Rubber ]\`.
- **Instant Console Command Execution**: Configured Minecraft containers with \`ENABLE_AUTOPAUSE=FALSE\` so the server thread never pauses when empty, executing console commands instantly in 0ms.
- **Deduplicated Console Output**: Eliminated double command echo (\`> pl\`) across Admin and User web terminals.
- **Enhanced Command Dispatcher**: Added automatic multiline parsing and \`mc-send-to-console\` fallback for all container commands.
...
async function main() {
  ...
  try {
    ...
    execSync(`git commit -m "Release ${TAG} - [ Rubber ] Console Identity, RCON Auto-Execution & Single Prompt Stream"`, { stdio: "inherit" });
  } catch (e) {
