...
const TAG = "v0.1.0-beta.51";
const NAME = "Rubber Panel v0.1.0-beta.51 — Console In Pipe, Multi-Stage RCON & Instant Command Execution";
const BODY = `## Rubber Panel v0.1.0-beta.51

### 🚀 Highlights & Fixes
- **Native Console In Pipe Support**: Enabled \`CREATE_CONSOLE_IN_PIPE=true\`, eliminating the missing \`/tmp/minecraft-console-in\` error.
- **4-Stage Resilient Command Dispatcher**:
  1. Direct \`rcon-cli\` via internal password
  2. Direct named pipe write to \`/tmp/minecraft-console-in\`
  3. Environment-inferred \`rcon-cli\`
  4. \`mc-send-to-console\` wrapper
- **Complete Autopause Disable**: Guarantees paper and vanilla servers never sleep, preventing command queues from stalling.
...
async function main() {
  ...
  try {
    ...
    execSync(`git commit -m "Release ${TAG} - Console In Pipe, Multi-Stage RCON & Instant Command Execution"`, { stdio: "inherit" });
  } catch (e) {
