...
const OLD_TAG = "v0.1.0-beta.53";
const TAG = "v0.1.0-beta.54";
const REPO = "Flaxa-Technologies/rubber-panel";
const NAME = "Rubber Panel v0.1.0-beta.54 — RCON Disabled by Default & Console Pipe-First Command Dispatch";
const BODY = `## Rubber Panel v0.1.0-beta.54

### 🚀 Highlights & Fixes
- **RCON Disabled by Default**: RCON is now fully opt-in. New servers will have \`enable-rcon=false\` in \`server.properties\` and \`ENABLE_RCON=false\` in Docker environment. This stops unwanted RCON port exposure.
- **Pipe-First Command Dispatch**: Console commands now go through the \`/tmp/minecraft-console-in\` named pipe (CREATE_CONSOLE_IN_PIPE) as the primary strategy, with \`mc-send-to-console\` and direct \`docker exec\` STDIN write as fallbacks — eliminating the RCON dependency for sending commands.
- **Slash Stripping**: Commands entered in the console panel with a leading \`/\` are now automatically stripped before being sent (Minecraft server console doesn't use \`/command\` syntax).

> **Important**: The \`pl\` command is a Bukkit/Paper shorthand for \`plugins\`. It should work on Paper servers. If you see \`Unknown or incomplete command\`, try the full \`plugins\` command. This is a server-side limitation (not a panel bug).
...
async function main() {
  ...
  try {
    ...
    execSync(`git commit -m "Release ${TAG} - RCON disabled by default, pipe-first command dispatch, slash stripping"`, { stdio: "inherit" });
