...
const OLD_TAG = "v0.1.0-beta.54";
const TAG = "v0.1.0-beta.55";
const REPO = "Flaxa-Technologies/rubber-panel";
const NAME = "Rubber Panel v0.1.0-beta.55 — Minecraft Server Software Version Provisioning Fix";
const BODY = `## Rubber Panel v0.1.0-beta.55

### 🚀 Highlights & Fixes
- **Minecraft Server Version & Software Provisioning Fix**:
  - Fixed User Portal and Admin Portal server creation to resolve selected software (\`PAPER\`, \`PURPUR\`, \`FABRIC\`, \`FORGE\`, etc.) and target version (\`1.21.1\`, \`1.21\`, etc.) directly from database.
  - Generates and persists the complete Docker \`environment\` payload (\`TYPE\`, \`VERSION\`, \`JAVA_VERSION\`, \`MEMORY\`, \`EULA\`, \`ENABLE_AUTOPAUSE\`) on the Server record and forwards it to the Node Daemon.
  - Fixed Node Daemon to enforce \`env.TYPE\` and \`env.VERSION\` during Docker container launch, preventing fallback to Vanilla Latest.
  - Updated Reinstall routes on both User and Admin sides to respect the configured software and version.
...
async function main() {
  ...
  try {
    ...
    execSync(`git commit -m "Release ${TAG} - Minecraft software type and version provisioning fix"`, { stdio: "inherit" });
