...
const OLD_TAG = "v0.1.0-beta.56";
const TAG = "v0.1.0-beta.57";
const REPO = "Flaxa-Technologies/rubber-panel";
const NAME = "Rubber Panel v0.1.0-beta.57 — Java LTS Docker Image Resolution Fix";
const BODY = `## Rubber Panel v0.1.0-beta.57

### 🚀 Highlights & Fixes
- **Enforced Java 21 LTS Image Tag**:
  - In \`itzg/minecraft-server\`, untagged \`latest\` maps to experimental Java 25 which fails to boot modern Paper builds.
  - Node Daemon and Admin APIs now explicitly map the Docker image tag to \`itzg/minecraft-server:java21\` (or \`java17\` / \`java8\` according to the selected Java runtime).
  - Added \`-i\` interactive terminal flag to Minecraft container execution for smooth console stream capture.
...
async function main() {
  ...
  try {
    ...
    execSync(`git commit -m "Release ${TAG} - Enforce Java 21 LTS image tag and interactive terminal"`, { stdio: "inherit" });
