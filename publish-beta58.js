...
const OLD_TAG = "v0.1.0-beta.57";
const TAG = "v0.1.0-beta.58";
const REPO = "Flaxa-Technologies/rubber-panel";
const NAME = "Rubber Panel v0.1.0-beta.58 — Cryo-Sleep Wake Deduplication & Live Console Streaming Fix";
const BODY = `## Rubber Panel v0.1.0-beta.58

### 🚀 Highlights & Fixes
- **Cryo-Sleep Wake Deduplication**:
  - Added in-flight lock to \`wakeServer\` to prevent duplicate rapid connection pings from triggering race conditions and prematurely stopping booting instances.
- **Console Log Stream Fix**:
  - Fixed log slice boundary conditions in \`node-side\` and \`ConsolePanel\` to ensure logs stream immediately to the user portal console when instances wake from Cryo-Sleep.
- **Java Version Setting Sync**:
  - Changing the Java Version on the Server Settings page now updates both \`JAVA_VERSION\` and \`DOCKER_IMAGE\` in the environment payload and dispatches a live configuration sync to the Node Daemon.
...
async function main() {
  ...
  try {
    ...
    execSync(`git commit -m "Release ${TAG} - Cryo-Sleep deduplication and live console log streaming"`, { stdio: "inherit" });
