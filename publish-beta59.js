...
const OLD_TAG = "v0.1.0-beta.58";
const TAG = "v0.1.0-beta.59";
const REPO = "Flaxa-Technologies/rubber-panel";
const NAME = "Rubber Panel v0.1.0-beta.59 — Java Version Switch & Active Log Stream Process Isolation";
const BODY = `## Rubber Panel v0.1.0-beta.59

### 🚀 Highlights & Fixes
- **Java Version Switching Fix**:
  - Fixed selection UI state in the Server Settings page to cleanly select and save Java version without mismatching Java 25.
  - Server Settings API now synchronizes \`javaVersion\`, \`javaVersionId\`, and \`DOCKER_IMAGE\` (\`itzg/minecraft-server:java\${cleanJava}\`) to ensure changes take effect immediately on container start.
  - Node Daemon auto-sanitizes any residual Java 25 tags if \`JAVA_VERSION\` is not 25.
- **Log Stream Process Management**:
  - \`attachLogs\` now tracks and terminates previous \`docker logs -f\` processes when containers restart or auto-wake from Cryo-Sleep.
...
async function main() {
  ...
  try {
    ...
    execSync(`git commit -m "Release ${TAG} - Java version switch and log stream isolation"`, { stdio: "inherit" });
