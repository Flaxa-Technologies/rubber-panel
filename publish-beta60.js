...
const OLD_TAG = "v0.1.0-beta.59";
const TAG = "v0.1.0-beta.60";
const REPO = "Flaxa-Technologies/rubber-panel";
const NAME = "Rubber Panel v0.1.0-beta.60 — Java Version Instant Auto-Save & Node Global Log Buffers";
const BODY = `## Rubber Panel v0.1.0-beta.60

### 🚀 Highlights & Fixes
- **Instant Auto-Save for Java Version Switch**:
  - Clicking any Java version on the Server Settings page now instantly selects and saves without waiting for "Save Changes".
  - Fixed background layout refresh polling so it no longer resets user selection state.
  - Added loading indicator directly on the selected Java card during saving.
- **Node Global In-Memory Ring Buffer**:
  - Bound \`consoleLogs\`, \`serverStates\`, and \`activeLogStreams\` to \`globalThis\` in Node Daemon to ensure all route handlers and background Cryo-Sleep proxy engines share the exact same active log buffer and child processes.
...
async function main() {
  ...
  try {
    ...
    execSync(`git commit -m "Release ${TAG} - Instant auto-save for Java runtime and global log streams"`, { stdio: "inherit" });
