...
const OLD_TAG = "v0.1.0-beta.60";
const TAG = "v0.1.0-beta.61";
const REPO = "Flaxa-Technologies/rubber-panel";
const NAME = "Rubber Panel v0.1.0-beta.61 — Verified LTS Java Tags & Client Chunk Recovery";
const BODY = `## Rubber Panel v0.1.0-beta.61

### 🚀 Highlights & Fixes
- **Verified Official LTS Java Docker Tags**:
  - Filtered out non-existent tags (\`java22\`, \`java23\`, \`java24\`, \`java25\`) that caused Docker daemon pull failure (\`docker: Error response from daemon: failed to resolve reference "docker.io/itzg/minecraft-server:java22": not found\`).
  - Standardized available runtimes to verified official tags: **Java 21 (Recommended LTS)**, **Java 17 (LTS)**, **Java 11 (LTS)**, **Java 8 (Legacy)**, and **Java 21 GraalVM**.
  - Node Daemon automatically sanitizes any legacy/invalid runtime strings to \`java21\`.
- **Client Deployment Chunk Auto-Recovery**:
  - Automatically reloads the web interface if a stale chunk hash from a previous panel update is encountered (\`Failed to load chunk /_next/static/...\`).
...
async function main() {
  ...
  try {
    ...
    execSync(`git commit -m "Release ${TAG} - Verified official LTS Java tags and chunk auto-recovery"`, { stdio: "inherit" });
